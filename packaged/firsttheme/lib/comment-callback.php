...
function firsttheme_comment_callback( $comment, $args, $depth) {
    ...
    <<?php echo $tag ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( ['c-comment', $comment->comment_parent ? 'c-comment--child' : ''] ) ?>>
        <article id="div-comment-<?php comment_ID(); ?>" class="c-comment__body">
            ...
            <?php edit_comment_link( esc_html__( 'Edit Comment', 'firsttheme' ), '<span class="c-comment__edit-link">', '</span>' ); ?>
            ...
            <div class="c-comment__content">
                ...
                <?php if($comment->comment_approved == '0') { ?>
                    <p class="c-comment__awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'firsttheme' ); ?></p>
                <?php } ?>
